...
private theorem computedConfigs_bounded_by_gates_flat
  ...
  -- Step 1: Prove FG node count equals gateDigests.length
  have h_fg_count : fgNodes.length = r.gateDigests.length := by
    -- For planted instances: gateReq is interval predicate [clause_start, clause_start + numGates)
    -- Use countP_finRange_interval to count gates in interval
    unfold fgNodes
    rw [h_L_eq]
    -- After substitution: need to show filter count on plant_flat equals r.gateDigests.length

    -- Unfold plant_flat to expose gateReq definition
    unfold plant_flat
    simp only []
    ...
    show clause_start + numGates ≤ (Construction.build3SATReductionDAG φ).n
    simp only [Construction.build3SATReductionDAG, Construction.totalNodes, Construction.reductionTreeSize]
    simp only [clause_start, numGates]
    have h_bound : r.gateDigests.length ≤ φ.clauses.length := h_wf.2.1
    omega
